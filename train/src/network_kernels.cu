...
extern float *data_svm;
extern float *data_diff;
extern int cnt;
...
float *get_network_output_layer_conv_gpu(network net)
{
    layer l = net.layers[18];
...
float *network_predict_gpu(network net, float *input)
{
    ...
    float *out_conv=get_network_output_layer_conv_gpu(net);
    freopen("out.txt","a",stdout);int i,j,k;
