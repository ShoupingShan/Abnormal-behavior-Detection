...
/////////////////////////////////////////gpu卷积////////////////////////////////////////////////////
/////////__global__ void write(float *output,float *output_gpu,int num){int i; for(i=0;i<num;i++){output[i]=output_gpu[i];}}
...
void forward_convolutional_layer_gpu(convolutional_layer l, network_state state)
{
   ...
   //cuda_pull_array(l.output_gpu, l.output2, l.outputs*l.batch);
//printf("outputs=%d   \n",l.outputs);
 //  cudaMemcpy(&l.output2,l.output_gpu,sizeof(float)*100,cudaMemcpyDeviceToHost);
 ////// write(l.output,l.output_gpu,l.outputs);
  //cudaMemcpyToSymbol(devData,&l.);
/*int shp;
     for(shp=0;shp<10;shp++)
    {l.output2[shp]=l.output_gpu[shp];}*/
    /*    freopen("out.txt","a",stdout);
 	printf("%f ",l.output2[0]);
